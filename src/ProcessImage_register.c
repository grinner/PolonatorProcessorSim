                            ...
                            int *offsety_history)
{
    ...
    *max_score = 0;
    ...
    if((abs(*offset_xcols) < MAX_ADDITIONAL_OFFSET)&&(abs(*offset_yrows) < MAX_ADDITIONAL_OFFSET))
    {
        if((abs(*offset_xcols - offset_start_x) < 9)||
            (abs(*offset_yrows - offset_start_y) < 9) ||
             (*max_score > 50000000))
        {
...
#ifdef DEBUG0
    fprintf(reglogfile, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", *offset_xcols, *offset_yrows, *max_score, offset_start_x, offset_start_y,
        ((-1*(search_xcols))+offset_start_x),
        ((-1*(search_yrows))+offset_start_y),
        search_xcols+offset_start_x,
        search_yrows+offset_start_y);
