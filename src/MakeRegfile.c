...
int main(int argc, char *argv[]){
    ...
    if(argc !=3 ){
        sprintf(log_string, "Usage: %s <Position file> = %s, <Number of flowcells> = %s, total arguement = %d", argv[0],argv[1],argv[2], argc);
        p_log(log_string);
        exit(0);
    }
    ...
    strcpy(obj_logfilename, argv[1]);
    strcat(obj_logfilename, ".obj_log");
    ...
    if( (obj_logfile=fopen(obj_logfilename, "w")) == NULL){
        sprintf(log_string, "ERROR opening reglog file %s for output", argv[2]);
        p_log_errorno(log_string);
        exit(0);
    }
    ...
    for(i=0; i<num_fcs; i++){
        for(j=0; j<ARRAYS_PER_FC; j++){
            for(k=0; k<IMGS_PER_ARRAY; k++){
                ...
                //
                // ------
                if ((strcmp("autoexp_pos", argv[1])==0)) //(k == 15) && 
                {
                    k = 2180;
                    sprintf(log_string, "finishing auto-exposure reg file, break out of nested loops");
                    p_log(log_string);
                    break;    
                }
                ...
                sprintf(log_string, "Position %d %d %d... %d objects\n", i, j, k, curr_numobjs);
                p_log_simple(log_string);
                sprintf(log_string, "%d\n", curr_numobjs);
