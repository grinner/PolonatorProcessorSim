...
int ProcessImage_new(short unsigned int *reg_pix_list_xcols,
                    short unsigned int *reg_pix_list_yrows,
                    FILE *full_pixel_list, 
                    short unsigned int *image,
                    short unsigned int *image_info,
    ...
    reg_pointer_offset = (curr_arraynum*15)* REG_PIXELS;

    sprintf(log_string, "STATUS:\tProcessImage_New: started operating on image %d, array %d, fc %d", 
        curr_imgnum,
        curr_arraynum,
        curr_fcnum);
    p_log_simple(log_string);
  ...
  //  fprintf(reglogfile, "=%d\t%d\t%d\t%d\t%d\n", curr_arraynum, curr_imgnum, offset_xcols, offset_yrows, score);
    ...
    gain_number = curr_imgnum*AUTO_EXPOSURE_STEP_SIZE +AUTO_EXPOSURE_STARTING_GAIN;

    fprintf (bead_value_file, "\n....... for lane %d, start extracting image = %d, with exposure (gain) = %d.........\n",curr_arraynum, curr_imgnum, gain_number);
    fprintf (bead_sum_file, "\n....... for lane %d, start extracting image = %d, with exposure (gain) = %d .........\n",curr_arraynum, curr_imgnum, gain_number);
    // PULL 'BEAD PIXELS' FROM IMAGE USING FULL MASK ON DISK
    ProcessImage_extract_new(image,
                offset_xcols,
                offset_yrows,
                *(image_info+3),
                full_pixel_list,
    ...
    //sprintf(log_string, "STATUS:\tProcessImage: image = %d, using gain = %d, acquired bead average value = %d", curr_imgnum, gain_number, array_index);
    sprintf(log_string, "STATUS:\tProcessImage: image = %d, using gain = %d, acquired bead average value = %llu", curr_imgnum, gain_number, array_index);
    p_log_simple(log_string);

    *array_value = array_index;

/*    bead_result_value[curr_imgnum] = array_index;
*/
    if(fwrite(beadvals, sizeof(short unsigned int), num_beads, beadfile) < num_beads){
        p_log_errorno((char*)"ERROR:\tProcessImage: write to bead file failed");
        exit(1);
    ...
    p_log((char*)"STATUS:\tProcessImage: writing output to files1");
